<template>
  <div class="ksd-logindrop" :class="{userlogin:flag,show:!flag,'xjy-header-items': after}" data-vip="1" :style="userLoginBg" @click="userLogin" @mouseleave="exchangeGrade">
    <a href="/user" class="ksd-home pr tp2" :style="userHead"><i
      class="iconfont icon-home pr pr-1"></i>进入主页</a>
    <a class="ksd-user-info" href="javascript:void(0);" :class="{'dropdown-toggle':after,'dropdown-toggle-other':!after}" :title="this.loginInfo.nickname" data-vip="1"
       id="navbarDropdown">
            <span class="pr">
    ...
    <div class="ksd-login-items bnone" :class="{'xjy-login-items': after}" style="border-bottom: none;">
      <ul>
        <li class="items">
          <a href="javascript:void(0);" class="fl flitems ksd-logout"><i class="iconfont icon-tcdl"></i>退出登录</a>
        </li>
      </ul>
    </div>
  </div>
