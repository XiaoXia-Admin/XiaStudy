<template>
  <div class="main-content ksd-topic-content">
    ...
    <div class="xjy-left">
      <a v-for="(item,index) in labelList" :key="index" :href="'/bbs?from=1&amp;searchKey=' + item"
         class="badge tagscolor mr-2" :class="badgeColor(index)" target="_blank"><i
        class="iconfont icon-biaoqian pr-2 pr tp1"></i><span style="font-weight: bold;">{{ item }}</span></a>
    </div>
  </div>
</template>
